...
def multi_head_attention(Q: np.ndarray, K: np.ndarray, V: np.ndarray,
    ...
    attention_output = attention_weights @ V_split
...
def encoder_block(x: np.ndarray, W_q: np.ndarray, W_k: np.ndarray, W_v: np.ndarray,
    ...
    """
    X_1 = layer_norm(x + multi_head_attention(x,x,x,W_q, W_k, W_v,W_o, num_heads),gamma1,beta1)
    return layer_norm(X_1 + feed_forward(X_1,W1,b1,W2,b2),gamma2,beta2)